...
import { FORK_AT, RUN_LENGTH, cardOf, isFork, type Run, type StopRecord } from './run'
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-1">
      {run.stops.map((stop, index) => {
        ...
        const label = isFork(stop) ? '⑂' : stop.number === RUN_LENGTH ? '👑' : String(stop.number)
        return (
          <span
            key={stop.number}
            title={`Stop ${stop.number}${stop.number === FORK_AT ? ' — fork' : ''}`}
